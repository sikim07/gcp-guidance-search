...
유료 합성을 켤 때도 GPT-5.4 nano 하나만 쓴다. 실사용 트래픽으로 검증한 숫자는 아니고, rate limit 상한(하루 전체 50건)을 한 달 내내 꽉 채웠다고 가정한 계산이다. 그 상한 기준으로도 비용은 1달러대라, 트래픽이 늘어도 예측 범위를 크게 벗어나지 않는다는 게 핵심이다.
