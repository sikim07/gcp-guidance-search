...
검색의 핵심은 랭킹이지 긴 추론이 아니다. 답변은 적재된 조항 2~3개를 근거로 두세 문장만 쓰면 충분하다. 그래서 채팅 API 키 없이도 제품이 완결되게 만들었고, 기본값도 "끔"으로 뒀다. 이유는 세 가지다.

오답 위험보다 발췌가 안전하다. 규제 조항은 원문 그대로 인용하는 게 맞다. LLM이 요약·재구성하면 없는 내용을 있는 것처럼 답하거나 표현이 미묘하게 바뀔 위험이 있고, 이건 비용보다 더 큰 리스크다.

비용을 예측할 수 없는 구조를 만들고 싶지 않았다. 개인이 무상으로 운영하는 프로젝트라, 트래픽이 갑자기 늘면 API 비용이 얼마가 나올지 알 수 없다. rate limit·캐싱으로 막아도 "기본값이 0원"인 편이 가장 안전하다.

발췌만으로도 실사용 가치가 이미 있다. 검색·랭킹 품질이 핵심이라, 조항을 그대로 찾아 보여주는 것만으로 CRA가 원문 사이트를 돌아다니는 문제는 이미 해결된다. 생성형 응답은 가독성을 더할 뿐, 없어도 제품이 완결된다.
